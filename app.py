...
def load_db():
    # نحاول نقرا من /tmp ولو فشل من الملف القديم
...
def home():
    ...
    <p>منصة الأمانة لإرجاع المفقودات</p><p style=font-size:12px>بإشراف وضمان: {ADMIN_NAME} - {ADMIN_PHONE}</p></div>
    <div class="box alert"><b>⚖️ نظام الأمانة:</b> الحاجة بتفضل مع اللي لاقيها، والتسليم برعاية مكتب محاماة.</div>
    <a href=/found class=btn style=background:#0d5a3c>😊 انا لقيت حاجة</a>
    <a href=/lost class=btn style=background:#c62828>😢 حاجة ضايعة مني</a>
    <a href=/all class=btn style=background:#263238>🔍 تصفح البلاغات ({len(active)})</a>
    <a href=/pay class=btn style="background:linear-gradient(135deg,#ff8f00,#ff3d00);color:#000">⭐ ثبت مفقوداتك ب 20 ج</a>
    <button id="installBtn" style="position:fixed;bottom:15px;left:50%;transform:translateX(-50%);width:90%;max-width:520px;background:#0d5a3c;color:#fff;padding:18px;border-radius:18px;border:2px solid #fff;font-weight:800;font-size:19px;z-index:999999;display:none">📲 ثبت التطبيق</button>
<script>let p=null;const b=document.getElementById('installBtn');window.addEventListener('beforeinstallprompt',(e)=>{{e.preventDefault();p=e;b.style.display='block';}});b.addEventListener('click',async()=>{{if(p){{p.prompt();await p.userChoice;p=null;b.style.display='none';}}}});</script></div>""")

@app.route("/terms")
def terms(): return page(f"""<div class=box><h2 style=text-align:center>⚖️ الشروط</h2><div style=line-height:2.2;font-size:14px>الإدارة وسيط فقط بإشراف {ADMIN_NAME} ولا تستلم أي مفقودات.</div><a href=/ class=btn style=background:#0d5a3c>موافق ✅</a></div>""")
...
def all_items():
    ...
    if not sorted_list: html+="<div class=box>لا يوجد بلاغات حاليا. جرب تنشر أول بلاغ.</div>"
    for it in sorted_list:
        ...
        <a class=btn style=background:#25D366 href='https://wa.me/20{ADMIN_PHONE[1:]}?text=بخصوص {it['type']} في {it['gov']}' target=_blank>تواصل مع الوسيط 💬</a>"""
        if is_admin:
...
@app.route("/pay")
def pay_page(): return page(f"""<div class=box style=text-align:center><h2>⭐ تثبيت 3 أيام</h2><div class=alert>حول 20 جنيه على فودافون كاش</div><div style=background:#f1f8e9;padding:20px;border-radius:15px;margin:15px 0><h1 style=color:#0d5a3c>{ADMIN_PHONE}</h1><p>باسم: {ADMIN_NAME}</p></div><a class=btn style=background:#25D366 href='https://wa.me/20{ADMIN_PHONE[1:]}?text=حولت ال20 جنيه' target=_blank>📸 ابعت سكرين التحويل</a><a href=/ class=btn style=background:#eee;color:#333>رجوع</a></div>""")
